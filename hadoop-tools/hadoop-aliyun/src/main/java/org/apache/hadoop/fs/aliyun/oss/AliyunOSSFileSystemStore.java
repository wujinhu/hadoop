...
public class AliyunOSSFileSystemStore {
  ...
  boolean isVersioningBucket() {
    return ossClient.getBucketVersioning(bucketName).getStatus() != null;
  }
  /**
  ...
  public RemoteIterator<LocatedFileStatus> createLocatedFileStatusIterator(
    ...
    return new RemoteIterator<LocatedFileStatus>() {
      ...
      public boolean hasNext() throws IOException {
        ...
        do {
          // for versioning buckets, maybe no files returned
          // in this batch, so we should continue listing.
          ...
          requestNextBatch();
          LOG.debug("After this batch, hasNext() = {}, meetEnd = {}",
              batchIterator.hasNext(), meetEnd);
        } while (true);
      }
